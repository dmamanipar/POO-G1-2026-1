...
public class ProductoRepository extends AbstractJpaRepository<Producto, Long> {
    ...
    protected Producto insert(Connection conn, Producto e) throws SQLException {
        long id = executeInsertGetKey(conn,
                "INSERT INTO upeu_producto(nombre,pu,puold,utilidad,stock,stockold,id_categoria,id_marca,id_unidad) VALUES(?,?,?,?,?,?,?,?,?)",
                e.getNombre(),
                e.getPu(),
                e.getPuold(),
                e.getUtilidad(),
    ...
    protected Producto updateRow(Connection conn, Producto e) throws SQLException {
        executeUpdate(conn,
                "UPDATE upeu_producto SET nombre=?,pu=?,puold=?,utilidad=?,stock=?,stockold=?,id_categoria=?,id_marca=?,id_unidad=? WHERE id_producto=?",
                e.getNombre(),
                e.getPu(),
                e.getPuold(),
                e.getUtilidad(),
